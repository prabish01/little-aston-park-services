...
function Stars({ rating }: { rating: number }) {
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((i) => (
        <svg key={i} className={`w-4 h-4 ${i <= Math.floor(rating) ? "text-yellow-400" : i - 0.5 <= rating ? "text-yellow-400" : "text-gray-300"}`} fill="currentColor" viewBox="0 0 20 20">
...
export default function TestimonialsSection() {
  return (
    <section id="testimonials-cards" className="relative w-full bg-[#f0f1eb] py-20 px-6 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto flex flex-col gap-10">
        ...
        <FadeIn className="flex flex-col gap-2">
          ...
          <h2 className="font-bold uppercase" style={{ fontSize: "clamp(1.6rem, 4vw, 2.8rem)", lineHeight: 1.05 }}>
            <span style={{ color: "#0d2e1a" }}>Hear From Our </span>
            <span style={{ color: "#3a8c52" }}>Happy Customers</span>
          </h2>
          <div className="text-gray-500 text-base leading-relaxed flex flex-col gap-4 max-w-4xl">
            <p>
              Don&apos;t just take our word for it, though. We have hundreds of happy customers, many of whom we&apos;ve developed long-term relationships with. We value feedback from all our customers, and our testimonials reflect the quality of our work, our reliability and the care and attention
              we put into every single project.
            </p>
        ...
        <FadeIn delay={0.1}>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {reviews.map((r) => (
              <div key={r.name} className="flex flex-col gap-4 rounded-2xl bg-white border border-gray-200/60 p-6">
                ...
                <div className="flex items-center gap-3 pt-2 border-t border-gray-200/80">
                  <div className={`w-10 h-10 rounded-full ${r.color} flex items-center justify-center text-white text-xs font-bold shrink-0`}>{r.initials}</div>
                  <div>
