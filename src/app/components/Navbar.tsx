  ...
  return (
    <>
      ...
      <header className={`fixed top-0 inset-x-0 z-50 flex justify-center pt-4 px-4 pointer-events-none transition-transform duration-300 ${hidden ? "-translate-y-full" : "translate-y-0"}`}>
        <div
          className={`pointer-events-auto flex items-center gap-1 px-2 py-2 rounded-full border transition-all duration-500 w-full max-w-7xl ${
            scrolled
              ? "bg-white/95 border-gray-200 shadow-xl shadow-black/10 backdrop-blur-xl"
              : "bg-white/75 border-white/50 shadow-lg shadow-black/5 backdrop-blur-md"
          }`}
        >
          {/* Logo */}
          <Link href="/" className="flex items-center px-2 mr-1">
            <Image src="/laps.svg" alt="Little Aston Park Services" width={38} height={38} className="h-9 w-auto" />
          </Link>
            ...
            {navLinks.map((link) =>
              link.label === "Services" ? (
                ...
                >
                  ...
                  {/* Services dropdown */}
                  <div
                    className={`absolute top-full left-1/2 -translate-x-1/2 mt-3 w-56 rounded-2xl bg-white border border-gray-100 shadow-2xl shadow-black/10 py-2 overflow-hidden transition-all duration-200 ${
                  ...
                  >
                    {serviceDropdownItems.map((item) => (
                      <Link
                        key={item.label}
                        href={item.href}
                        className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-600 hover:text-emerald-700 hover:bg-emerald-50 transition-colors"
                      >
          ...
          <Link
            href="/contact"
            className="hidden md:flex items-center gap-2 px-5 py-2 rounded-full bg-emerald-600 text-white text-sm font-semibold hover:bg-emerald-500 transition-colors"
          >
            Get a Quote
          ...
          {/* Mobile — burger */}
          <button
            className="md:hidden p-2 rounded-full text-gray-600 hover:bg-gray-100/70 transition-colors"
