...
export default function LawnMowerStrip() {
  ...
  return (
    <div
      ...
      style={{ height: "clamp(80px, 14vw, 130px)", background: "#e9ede5" }}
      aria-hidden="true"
    >
      {/* CSS for grass sway */}
      <style>{`
      ...
      >
        ...
        {blades.map((b, i) => {
          ...
          const bx = b.x, by = BASE_Y, ty = r(tipY);
          const path = [
            `M ${bx} ${by}`,
          ...
          return (
            <g
              ...
              style={{
                animation: trimmed
                  ? "none"
                  : `${animName} ${b.swayDur}s ease-in-out ${b.swayDelay}s infinite`,
            ...
            >
              <path
                d={path}
                fill={trimmed ? "url(#grTrimmed)" : `url(#gr${i % COLORS.length})`}
                opacity={trimmed ? 0.75 : 0.92}
              />
            </g>
          );
        })}
        ...
        {blades
          ...
          .map((b, i) => (
            <ellipse
              key={`clip-${b.id}`}
              cx={b.x + (sr(i * 7) - 0.5) * 16}
              cy={BASE_Y - 2 - sr(i * 11) * 7}
              rx={2 + sr(i * 3) * 3}
              ry={1 + sr(i * 5) * 1.5}
              fill="#4ade80"
              opacity={0.35 + sr(i * 17) * 0.25}
              transform={`rotate(${(sr(i * 13) - 0.5) * 60} ${b.x} ${BASE_Y - 4})`}
            />
          ))}
        ...
        <g transform={`translate(${mowerX}, ${BASE_Y - 58})`}>
          ...
          {[0,90,180,270].map((deg) => (
            <line key={deg}
              x1={12 + Math.cos(deg*Math.PI/180)*2.5}
              y1={52 + Math.sin(deg*Math.PI/180)*2.5}
              x2={12 + Math.cos(deg*Math.PI/180)*5.5}
              y2={52 + Math.sin(deg*Math.PI/180)*5.5}
          ...
          {[0,90,180,270].map((deg) => (
            <line key={deg}
              x1={54 + Math.cos(deg*Math.PI/180)*2.5}
              y1={52 + Math.sin(deg*Math.PI/180)*2.5}
              x2={54 + Math.cos(deg*Math.PI/180)*5.5}
              y2={52 + Math.sin(deg*Math.PI/180)*5.5}
              stroke="#9ca3af" strokeWidth="1.2" strokeLinecap="round"
            />
          ))}
