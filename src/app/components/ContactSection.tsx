  ...
  return (
    <section id="contact" className="relative w-full overflow-hidden bg-[#f8f9f3]">
      ...
      <div
        ...
        style={{ background: "linear-gradient(135deg, #0f3320 0%, #1a4731 40%, #2d7a4f 100%)" }}
      >
        {/* Decorative circles */}
        ...
        <div className="relative z-10 max-w-7xl mx-auto flex flex-col lg:flex-row lg:items-end justify-between gap-8">
          <FadeIn className="flex flex-col gap-4 max-w-2xl">
            ...
            <h1 className="font-bold" style={{ fontSize: "clamp(2rem, 5vw, 3.5rem)", lineHeight: 1.08, color: "#ffffff" }}>
              Your Garden<br />
              <span style={{ color: "#6ee7b7" }}>Deserves Better</span>
            </h1>
      ...
      <div className="max-w-7xl mx-auto px-6 py-16 grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 items-start">
        ...
        <FadeIn className="flex flex-col gap-4 lg:sticky lg:top-28">
          ...
          <div className="flex flex-col gap-3">
            {contactItems.map((item) => (
              <div key={item.label} className="flex items-center gap-4 p-4 rounded-2xl bg-white border border-gray-100 shadow-sm">
                <span className="w-10 h-10 rounded-xl bg-emerald-50 flex items-center justify-center text-emerald-600 shrink-0">
                  {item.icon}
                </span>
                <div>
                  <p className="text-gray-400 text-xs font-medium">{item.label}</p>
                  {item.href ? (
                    <a href={item.href} className="text-emerald-950 font-semibold text-sm hover:text-emerald-600 transition-colors">{item.value}</a>
                  ) : (
          ...
          <div className="flex items-center gap-3 px-5 py-4 rounded-2xl bg-emerald-50 border border-emerald-100">
            ...
            <p className="text-emerald-800 text-sm font-medium">We typically respond within <strong>24 hours</strong></p>
          </div>
        </FadeIn>
        ...
        <FadeIn delay={0.1} className="lg:col-span-2">
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="px-8 py-6 border-b border-gray-100 flex items-center justify-between">
              ...
              <span className="hidden sm:flex items-center gap-1.5 text-xs font-semibold text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-full border border-emerald-100">
                Free quote
              </span>
            </div>
            ...
            <div className="p-8">
              {submitted ? (
                <div className="flex flex-col items-center justify-center gap-4 py-12 text-center">
                  ...
                  <button onClick={() => { setSubmitted(false); setForm({ name: "", email: "", phone: "", message: "" }); }} className="mt-2 text-sm font-semibold text-emerald-600 hover:text-emerald-800 transition-colors">
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Full Name <span className="text-emerald-500">*</span></label>
                      <input
                  ...
                  {/* Email */}
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Email Address <span className="text-emerald-500">*</span></label>
                    <input
                  ...
                  {/* Message */}
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Your Message <span className="text-emerald-500">*</span></label>
                    <textarea
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pt-1">
                    <p className="text-gray-400 text-xs">Fields marked <span className="text-emerald-500 font-semibold">*</span> are required. We&apos;ll never share your details.</p>
                    <button
                      type="submit"
                      className="shrink-0 flex items-center gap-2 px-8 py-3.5 rounded-full bg-emerald-600 hover:bg-emerald-500 transition-colors text-white font-semibold text-sm"
                    >
                      Send Message
